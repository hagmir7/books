    ...
    <div class="mx-auto px-4">
        <div class="flex flex-wrap text-center justify-center">
            <div class="py-12 w-full md:w-1/2">
                <h1 class="text-2xl font-semibold mb-6 w-full text-center">{{ $site->site_options['blog_title'] }}</h1>
                <form action="/blog" method="GET">
                    <div class="flex rounded-full border-2 border-gray-300 overflow-hidden bg-white shadow-sm">
                        ...
                        <input type="search" name="q"
                            class="flex-1 px-4 py-3 rounded-r-full focus:outline-none f rtl:rounded-r-none rtl:rounded-l-full"
    ...
    <div class="post bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow">
        ...
        <div class="post-info p-4">
            ...
            <a href="{{ route('blog.show', $post->slug) }}" class="post-title block">
                <h2 class="text-xl font-semibold text-gray-900 hover:text-primary transition-colors line-clamp-2">
                    {{ $post->title }}
                </h2>
            </a>
        </div>
